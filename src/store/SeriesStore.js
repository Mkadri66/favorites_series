...
const mutations = {
  ...
  DELETE_FAVORITES(state, { deleteSerie }) {
    ...
    this.state.favorites = state.favorites.filter(
      fav => fav.id !== deleteSerie.id
    );
  }
};
      ...
      return {
        ...
        url:
          data.poster_path === null
            ? require("../assets/no-image-fav-series.png")
            : "https://image.tmdb.org/t/p/w500/" + data.poster_path,
        overview:
          data.overview === ""
            ? "Il n'y pas de description pour cette serie."
            : data.overview,
        isFavorite: state.favorites.find(favorite => favorite.id == data.id)
          ? true
          : false
      };
    });
